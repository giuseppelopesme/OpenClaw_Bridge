#!/usr/bin/env python3
"""CLI for the agent-draft approval flow.

Subcommands:

    brain-drafts.py list   [--all] [--agent NAME]
    brain-drafts.py show   <draft_id>
    brain-drafts.py approve <draft_id> [--by NAME]
    brain-drafts.py reject  <draft_id> [--reason TEXT]
    brain-drafts.py retry   <draft_id>
    brain-drafts.py edit    <draft_id>      (opens $EDITOR)

Env:

    BRIDGE_URL   default http://127.0.0.1:8788
    CLI_TOKEN    bearer with `agent:drafts:read` + `agent:drafts:approve`
                 (mint via scripts/mint-token.py --actor cli.<account>
...
# The CLI is invoked via `uv run --no-sync python scripts/brain-drafts.py`,
...
def _token() -> str:
    ...
    if not token:
        sys.stderr.write(
            ...
            "  scripts/mint-token.py --actor cli.<your-account-name> "
            "--scopes agent:drafts:read,agent:drafts:approve\n"
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="brain-drafts",
        description="CLI for the bridge's agent-draft approval flow.",
    )
    ...
    p_list.add_argument(
        ...
        type=str,
        default=None,
        help="filter by agent name (free-form well-formed identifier)",
    )
    p_list.add_argument(
        "--limit",
        type=int,
