...
/v1/events/publish, and /v1/agent/drafts. P1a pivot: CLU now POSTs the
draft to the bridge via brains_shared.agent.create_draft instead of
storing it locally; the bridge owns publishing draft.pending.
"""
...
@pytest.fixture
def cfg(tmp_path: Path) -> CluConfig:
    return CluConfig(
...
async def test_already_processed_event_short_circuits(
    ...
    await state.mark_processed("evt-1", "imessage.received.clu")
...
async def test_llm_error_publishes_task_completed_with_error(
    ...
    completed = [t for t in captured if t[0] == "agent.clu.task.completed"]
...
async def test_create_draft_failure_publishes_error(
    ...
    """If the bridge rejects POST /v1/agent/drafts, CLU treats the whole
    handler as a failure: task.completed outcome=error, mark processed."""
    captured: list[tuple[str, dict[str, Any]]] = []
