...
CLU's only long-lived state. One table:
...
Drafts used to live here too. They moved to the bridge in P1a (the
bridge owns the approval lifecycle now). CLU just calls
``brains_shared.agent.create_draft`` after an LLM pass; the operator
manages the draft via ``scripts/clu-drafts.py`` against the bridge's
HTTP API. CLU is purely a draft *producer* now.

Connection model: one ``sqlite3.Connection`` per CLU process. SQLite is
fast enough that the threading bridge from the bridge's design isn't
worth it here (Session 2 made the same call for telemetry). All public
functions are async wrappers via ``asyncio.to_thread`` so the brain's
