"""Handler for ``imessage.received.clu`` envelopes.

P1a flow (bridge owns drafts):
...
4. On ``draft``: ask the LLM for a reply body, then call
   ...
   ``agent.clu.draft.pending`` on success.
5. Publish ``agent.clu.task.completed`` with the right outcome.
...
logger = logging.getLogger("clu.handlers.imessage_received")

_TRIAGE_SYSTEM = (
    "You are CLU, a personal assistant agent. Decide whether to draft a "
...
_DRAFT_SYSTEM = (
    "You are CLU, drafting a reply on behalf of the operator. Keep the "
...
async def handle(envelope: EventEnvelope, ctx: BrainContext) -> None:
    ...
    try:
        ...
        # `agent.clu.draft.pending`, and waits for the operator's PATCH
        # /v1/agent/drafts/{id}.  CLU is purely a draft producer.
...
async def _publish_task_completed(
    ...
    *,
    ...
    try:
        await publish_event(
            ctx.client,
            topic="agent.clu.task.completed",
            payload=payload,
        )
