"""End-to-end tests for scripts/brain-drafts.py against an in-process bridge.
...
We import the CLI by file path because ``scripts/brain-drafts.py`` has a
...
# Load scripts/brain-drafts.py as a module (hyphen → underscore alias).
_CLI_PATH = Path(__file__).resolve().parents[3] / "scripts" / "brain-drafts.py"
_spec = importlib.util.spec_from_file_location("brain_drafts_cli", _CLI_PATH)
assert _spec is not None and _spec.loader is not None
brain_drafts_cli = importlib.util.module_from_spec(_spec)
sys.modules["brain_drafts_cli"] = brain_drafts_cli
_spec.loader.exec_module(brain_drafts_cli)
...
def cli_env(
    ...
    real_bridge_client = brain_drafts_cli.BridgeClient
    ...
    monkeypatch.setattr(brain_drafts_cli, "BridgeClient", _PatchedBridgeClient)
    return client
...
def _run_cli(*argv: str) -> tuple[int, str, str]:
    ...
    with redirect_stdout(out), redirect_stderr(err):
        rc = brain_drafts_cli.main(list(argv))
